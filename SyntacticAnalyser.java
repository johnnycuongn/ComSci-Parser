...
public class SyntacticAnalyser {
	...
	public static Symbol[][] rules = {
			...
			{TreeNode.Label.charexpr, Token.TokenType.SQUOTE, Token.TokenType.CHARLIT, Token.TokenType.SQUOTE},
	...
	public static void findFirstSet() {
		...
		for (int i = 0; i < rules.length; i++) {
			...
			firstMap.get(rules[i][0]).add(rules[i][1]);
		}
...
//
	public static void findFollowSet(){
		...
		for (int i = 0; i < rules.length ; i++) {
			Symbol[] grammar = rules[i];
			for (int j = 1; j < grammar.length ; j++) {
				if (!grammar[j].isVariable()) {continue;}
				
				if(!followMap.containsKey(grammar[j])){
					followMap.put(grammar[j],new ArrayList<>());
				}


				if (j < grammar.length - 1) {
					if (firstMap.get(grammar[j + 1]) != null) {
						followMap.get(grammar[j]).addAll(firstMap.get(grammar[j + 1]));
						if (isNullable(grammar[j + 1]) && followMap.get(grammar[0]) != null && !grammar[j].equals(grammar[0]))
							followMap.get(grammar[j]).addAll(followMap.get(grammar[0]));
					}
				}
				else if (j == grammar.length - 1 && !grammar[j].equals(grammar[0])) {
					followMap.get(grammar[j]).addAll(followMap.get(grammar[0]));
				}
			}
		}
		...
		for (int i = 0; i < tokens.size(); i++) {
			...
			if (stack.peek().equals(Token.TokenType.ID)) {
				...
				if (tokens.get(i).getType().equals(Token.TokenType.ID)) {
					for (int k = 0; k < parent.getChildren().size(); k++) {
						if (parent.getChildren().get(k).getLabel().equals(TreeNode.Label.terminal) 
						&& parent.getChildren().get(k).getToken().get().getType().equals(Token.TokenType.ID)) {
							parent = parent.getChildren().get(k);
							parent.updateToken(tokens.get(i));
							break;
						}
